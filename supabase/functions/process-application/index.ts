...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (record.has_co_applicant) {
      ...
      if (coInsertError) {
        // Non-fatal: log the error but don't block the application submission.
        // The application record is already committed; a missing co-applicant row
        // is recoverable by admin, but a failed submission is not.
        console.error(`co_applicants insert failed for ${appId}:`, coInsertError.message)
      }
    }
    ...
    if (record.has_co_applicant && coApplicantEmail) {
      ...
      await supabase.from('email_logs').insert({ type: 'co_applicant_notification', recipient: coApplicantEmail, status: 'pending', app_id: appId })
      fetch(gasUrl, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ secret: gasSecret, template: 'co_applicant_notification', to: coApplicantEmail, data: coApplicantPayload })
      }).then(async (r) => {
        const json = await r.json().catch(() => ({}))
        const ok = r.ok && json.success !== false
        await supabase.from('email_logs').insert({ type: 'co_applicant_notification', recipient: coApplicantEmail, status: ok ? 'success' : 'failed', app_id: appId, error_msg: ok ? null : (json.error || `HTTP ${r.status}`) })
      }).catch(async (e) => {
        await supabase.from('email_logs').insert({ type: 'co_applicant_notification', recipient: coApplicantEmail, status: 'failed', app_id: appId, error_msg: e?.message || 'Network error' })
      })
    }
